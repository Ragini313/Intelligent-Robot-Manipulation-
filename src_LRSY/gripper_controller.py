...
class PickController(object):
    def __init__(self) -> None:
        ...
        self.gripper_move_result_subscriber = rospy.Subscriber(
            "/franka_gripper/move/result", MoveActionResult, self.move_result_callback
        )
        self.gripper_move_publisher = rospy.Publisher(
            "franka_gripper/move/goal", MoveActionGoal, queue_size=10
        )

        self.gripper_grasp_result = GraspActionResult()
        self.gripper_grasp_result_subscriber = rospy.Subscriber(
            "/franka_gripper/grasp/result",
            GraspActionResult,
            self.grasp_result_callback,
        )
        self.gripper_grasp_publisher = rospy.Publisher(
            "franka_gripper/grasp/goal", GraspActionGoal, queue_size=10
        )

        self.joint_state_subscriber = rospy.Subscriber(
            "/franka_gripper/joint_states", JointState, self.joint_state_callback
        )
    ...
    def grasp(self):
        ...
        grasp_action_goal.goal.width = 0.040  # 0.05

        grasp_action_goal.goal.epsilon.inner = 0.05  # 0.005
        grasp_action_goal.goal.epsilon.outer = 0.05

        grasp_action_goal.goal.speed = 0.15  # 0.15
        grasp_action_goal.goal.force = 15  # 20.0  # in Newton
