...
class CubeDetector(object):
    def __init__(self):
        ...
        self._debug_image_publisher = rospy.Publisher(f"preprocess_image", Image, queue_size=10)
    ...
    def _to_polygone(self, image):
        ...
        polygon = np.array([[150, 0], [540, 0], [540, 350], [150, 350]]) # [[150, 100], [500, 110], [480, 300], [180, 300]]
    ...
    def _transform_cube_point(self, point):
        try:
            ...
            transformed_point.header.frame_id = self._camera_frame_id
            transformed_point.header.stamp = rospy.Time(0)
    ...
    def _publish_cubes(self) -> None:
        if self._cubes:
            for cube in self._cubes:
                ...
                quats = tft.quaternion_from_euler(0, 0, math.radians(cube.rotation))
                ...
                print(f"cubeID: {cube.cube_id}", cube_pose)
    ...
    def run(self):
        if self._image is not None and self._depth_image is not None:
            try:
                ...
                self._debug_image_publisher.publish(self._cv_bridge.cv2_to_imgmsg(thresholded_image))
